        ...
        <div className="min-h-screen flex flex-col md:flex-row bg-gray-50">
            ...
            <aside className={`md:w-64 bg-white border-r px-6 py-8 ${showMobileMenu ? 'block' : 'hidden'} md:block`}>
                ...
                <nav className="space-y-4 text-gray-700">
                    ...
                    <a href="/admin/articles" className="block hover:text-indigo-600 bg-gray-100 p-2">Contenu</a>
            ...
            <main className="flex-1 p-4 sm:p-6 lg:p-8">
                ...
                <div className="mb-4 flex flex-wrap gap-2">
                    ...
                    <button className="px-3 py-1 text-sm border text-gray-500 rounded-md bg-white hover:bg-gray-100" onClick={() => fetchByDate(365)}>12 mois</button>
                    <button className="px-3 py-1 text-sm border text-gray-500 rounded-md bg-white hover:bg-gray-100" onClick={() => fetchByDate(30)}>30 jours</button>
                    <button className="px-3 py-1 text-sm border text-gray-500 rounded-md bg-white hover:bg-gray-100" onClick={() => fetchByDate(7)}>7 jours</button>
                    <button className="px-3 py-1 text-sm border text-gray-500 rounded-md bg-white hover:bg-gray-100" onClick={() => fetchByDate(1)}>24 heures</button>
                    <select
                    ...
                    {allArticles.map((article) => {
                        ...
                        return (
                            <div key={article.id} className="bg-white border rounded-lg p-4 shadow-sm">
                                {imageMedia ? (
                                    <img src={imageMedia.url} alt="Image de l'article" className="h-32 w-full object-cover rounded mb-4" />
                                ) : (
                                    <div className="h-32 bg-gray-200 rounded mb-4" />
                                )}
                                <p className="text-indigo-600 text-sm font-medium mb-1">
                                    {article.articles_created_by_fkey?.nickname ?? 'Auteur inconnu'} • {new Date(article.created_at).toLocaleString("fr-FR", {
                                        day: "2-digit",
                                        month: "long",
                                        year: "numeric",
                                        hour: "2-digit",
